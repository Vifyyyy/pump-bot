            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(MEXC_FUTURES_URL, timeout=15) as response:
                    if response.status == 200:
                        ...
                        # MEXC повертає список
                        if isinstance(data, list):
                            tickers = data
                            
                            # Збираємо всі USDT пари (ф'ючерси)
                            if not all_symbols:
                                all_symbols = [t.get('symbol', '') for t in tickers if t.get('symbol', '').endswith('USDT')]
                                print(f"📋 ✅ Знайдено {len(all_symbols)} ф'ючерсних пар (USDT-M)")
                                ...
                                # Відправляємо повідомлення про успішний запуск
                                await bot.send_message(
                                    chat_id=CHAT_ID,
                                    text=f"""🤖 **PUMP/DUMP Бот (MEXC Futures) запущено!**

📊 **Моніторинг:** {len(all_symbols)} ф'ючерсних пар
...
async def main():
    ...
    try:
        await bot.send_message(
            chat_id=CHAT_ID,
            text="🤖 **MEXC Futures Bot запускається...**\n\n📡 Завантажую список ф'ючерсних пар...",
            parse_mode='Markdown'
        )
